...
class UnsplashAuth extends NetworkBase implements UnsplashAuthInterface {
  ...
  protected function initSdk() {
    ...
    if (!class_exists($class_name)) {
      throw new SocialApiException(sprintf('The Unsplash Library for the league oAuth not found. Class: %s.', $class_name));
    }
    ...
    if ($this->validateConfig($settings)) {
      if ($proxyUrl) {
        $league_settings = [
          ...
          'redirectUri' => $this->requestContext->getCompleteBaseUrl() . '/user/login/unsplash/callback',
          'response_type' => 'code',
          'accessType' => 'offline',
          'verify' => FALSE,
          'proxy' => $proxyUrl,
